...
def load_jsonl(input_path):
    """
    Loads a JSONL (JSON Lines) file and shuffles the entries.
...
def convert_huggingface_data_to_list_dic(dataset):
    """
    Converts a Hugging Face dataset object into a list of dictionaries.
...
def get_metrics(scores, labels):
    """
    Calculates AUROC, FPR@95%TPR, TPR@5%FPR, and max accuracy.

    Args:
        scores (List[float]): Model scores or predictions.
        labels (List[int]): Ground truth binary labels.

    Returns:
        Tuple[float, float, float, float]: AUROC, FPR@95%TPR, TPR@5%FPR, and accuracy.
    """
...
def do_plot(prediction, answers, legend="", output_dir=None):
    """
    Generates a ROC curve for a single metric and returns performance scores.

    Args:
        prediction (List[float]): Predicted scores.
        answers (List[int]): Ground truth labels.
        legend (str): Label for the legend in the plot.
        output_dir (str, optional): Output directory for saving plots.

    Returns:
        Tuple[str, float, float, float]: Legend, AUROC, accuracy, TPR@5%FPR.
    """
...
def fig_fpr_tpr(all_output, output_dir):
    """
    Plots FPR/TPR curves for multiple metrics and writes summary to a file.

    Args:
        all_output (List[dict]): List of outputs containing "label" and "pred" dicts.
        output_dir (str): Directory to save figures and summary.
    """
...
def save_metrics_to_csv(results, output_dir, prefix):
    """
    Saves selected metric results to a CSV file.

    Args:
        results (dict): Dictionary of results with 'method', 'auroc', etc.
        output_dir (str): Directory to save the file.
        prefix (str): Prefix to use in the output filename.
    """
...
def convert_data_format(data):
    """
    Converts evaluation data into a format usable by `evaluate_and_save_results`.

    Args:
        data (List[dict]): List of data points with 'label' and 'pred'.

    Returns:
        dict: Dictionary mapping methods to their prediction scores.
    """
...
def write_to_csv_pred_min_k(data, filename):
    ...
    Args:
        data (List[dict]): A list of prediction result dictionaries.
...
def evaluate_and_save_results(scores_dict, data, dataset, folder, prefix=""):
    ...
    Args:
        scores_dict (dict): A dictionary of {metric_name: List of scores}.
        data (List[dict]): A list of dicts containing labels.
...
def create_combined_roc_curve_plot(roc_data, output_dir, title='ROC curve', prefix=""):
    """
    Creates and saves a combined ROC curve for multiple metrics.

    Args:
        roc_data (dict): A dictionary mapping method names to (fpr, tpr, auroc).
        output_dir (str): Directory to save the plot.
        title (str): Title of the plot.
        prefix (str, optional): Filename prefix for the saved image.
    """
...
def evaluate_like_min_k(csv_path, kind=''):
    """
    Loads a CSV of prediction scores and evaluates multiple detection metrics.

    Args:
        csv_path (str): Path to the CSV file.
        kind (str): Optional suffix/prefix to add to result file names.
    """
